(function ($) {
	...
	$.form = function(selector, context) {
		 ...
		 *  validcheck:boolean -> true : _f.check() false : 체크하지않음
		 * }
		 */
		_f.submit = function(p) {
			p = $.extend(false, { target:"_self", iframe: false, valid: null, confirm: null, validcheck: true }, p || {});
			return this.each(function() {
				...
				if (p.iframe) {
					...
					if ($("#_IFRAME", f).length == 0) {
						var hidden1 = $("<input />").attr("type", "hidden").attr("name", "_IFRAME").attr("id", "_IFRAME").val("Y");
						hidden1.appendTo(f);
						var hidden2 = $("<input />").attr("type", "hidden").attr("name", "_CALL_BACK_YN").attr("id", "_CALL_BACK_YN").val("Y");
						hidden2.appendTo(f);
					} else {
						$("#_IFRAME", f).val("Y");
						$("#_CALL_BACK_YN", f).val("Y");	
					};
				} else {
					$("#_IFRAME", f).remove();
				};
